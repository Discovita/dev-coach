...
describe("getCoachingPhaseDisplayName", () => {
  ...
  it("returns the correct name for specific phases", () => {
    expect(getCoachingPhaseDisplayName(CoachingPhase.INTRODUCTION)).toBe("Introduction");
    expect(getCoachingPhaseDisplayName(CoachingPhase.IDENTITY_BRAINSTORMING)).toBe("Identity Brainstorming");
    expect(getCoachingPhaseDisplayName(CoachingPhase.I_AM_STATEMENT)).toBe("I Am Statement");
  });

  it("returns the raw value as fallback for unknown phase", () => {
    expect(getCoachingPhaseDisplayName("unknown_phase" as CoachingPhase)).toBe("unknown_phase");
  });
