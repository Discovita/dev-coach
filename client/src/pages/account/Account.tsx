	...
	const { data: impersonatedProfile } = useQuery<User>({
		...
		queryFn: () => {
			if (!impersonatedUser?.id) {
				throw new Error("Cannot fetch impersonated profile without a user id");
			}
			return fetchTestScenarioUserProfile(impersonatedUser.id);
		},
			...
			<Tabs defaultValue="account">
				...
				<TabsContent value="reference-photos">
					<ReferenceImageManager
						userId={isImpersonating ? (viewedUserId ?? undefined) : undefined}
					/>
				</TabsContent>
