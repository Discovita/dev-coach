...
interface ReferenceImageUploaderProps {
	/** Optional user ID for admin impersonation. */
	userId?: string;
	/** Heading shown above the count. Defaults to "Reference Photos". */
	heading?: ReactNode;
	/** Intro copy shown under the heading. Pass null to hide it. */
	intro?: ReactNode;
}
 ...
 * --------------------------------
 * The reusable upload core for reference photos: heading + count, guidance,
 * and a drag-and-drop / file-picker grid for up to 5 photos. Auto-persists each
 * upload (and deletion) via the {@link useReferenceImages} hook, so it works
 * standalone in any container.
 *
 * Renders no card chrome of its own — wrap it in a card (account page) or a
 * page section (onboarding) as needed. `heading` / `intro` let callers tailor
 * the framing copy.
 *
