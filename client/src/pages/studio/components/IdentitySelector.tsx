...
export function IdentitySelector({
  ...
  if (isLoading) {
    return (
      <div className="text-[var(--nv-royal-purple)]/60">Loading identities...</div>
    );
  }
                ...
                <div className="flex items-center gap-2 w-full pr-6">
                  ...
                  >
                    {identityHasImage(identity) ? (
                      ...
                      <CircleDashed
                        className="size-4 text-[var(--nv-royal-purple)]/40"
                      />
                    )}
                  </span>
        ...
        <div className="mt-4 p-6 bg-[var(--nv-pale-lavender)]/30 rounded-lg border border-[var(--nv-royal-purple)]/20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {(selectedIdentity.image?.original || selectedIdentity.image?.large || selectedIdentity.image?.medium || selectedIdentity.image?.thumbnail) ? (
                <>
                  <img
                    ...
                    style={{ minHeight: '300px', objectFit: 'contain', backgroundColor: 'var(--nv-lilac-white)' }}
                  />
                  <p className="text-sm text-[var(--nv-royal-purple)]/60 mt-2">Current Image</p>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                ...
                <div className="w-full aspect-video bg-[var(--nv-lilac-white)] rounded-lg flex items-center justify-center min-h-[300px] border border-[var(--nv-royal-purple)]/20">
                  <p className="text-[var(--nv-royal-purple)]/60 text-lg">No image generated yet</p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col justify-center">
              <h3 className="font-bold text-2xl mb-4 text-[var(--nv-indigo)]">{selectedIdentity.name}</h3>
              {selectedIdentity.category && (
                <p className="text-base text-[var(--nv-royal-purple)]/80 mb-4">
                  <span className="font-semibold">Category:</span>{" "}
                  {getIdentityCategoryDisplayName(String(selectedIdentity.category))}
                </p>
              )}
