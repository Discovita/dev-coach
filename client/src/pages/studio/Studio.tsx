...
export default function Studio() {
	...
	const [generatedImageBase64, setGeneratedImageBase64] = useState<
		string | null
	>(null);
	const [isCurrentIdentityImageOpen, setIsCurrentIdentityImageOpen] =
		useState(false);
	/**
	...
	const selectedIdentity =
		identities?.find((id: Identity) => id.id === selectedIdentityId) || null;
	...
	const handleGenerate = async () => {
		...
		if (!hasReferenceImages) {
			toast.error(
				"Please upload at least one reference image in your Account settings before generating",
			);
			return;
		}
	...
	const canEdit =
		generatedImageBase64 && editPrompt.trim() && !isContinuingChat;
	...
	return (
		<div className="flex flex-col h-full w-full overflow-y-auto">
			<div className="mb-6">
				<h1 className="text-3xl font-bold text-[var(--nv-indigo)]">
					Identity Image Studio
				</h1>
				<p className="text-sm text-[var(--nv-royal-purple)]/70 mt-2 max-w-3xl">
					Pick an identity, describe the scene, generate a draft, then refine it
					with edits.
				</p>
			</div>
									...
									<div className="border border-[var(--nv-royal-purple)]/20 rounded-lg bg-[var(--nv-pale-lavender)]/20 p-4">
										...
										<div className="mt-3 space-y-3">
											...
											<button
												...
												aria-label={"Open current identity image"}
											>
												<div className="w-full aspect-video rounded-lg overflow-hidden border border-[var(--nv-royal-purple)]/20 bg-[var(--nv-lilac-white)]">
													<img
														...
														alt={`${selectedIdentity.name} current identity`}
						...
						<div className="border border-[var(--nv-royal-purple)]/20 rounded-lg bg-[var(--nv-pale-lavender)]/30 p-4">
							<div className="flex flex-col gap-4">
								<div className="flex items-start justify-between gap-4">
									<div className="min-w-0">
										<h2 className="text-xl font-semibold text-[var(--nv-indigo)]">
											Preview
										</h2>
										{selectedIdentity ? (
								...
								{selectedIdentityId && sceneIsDirty && (
									<div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
										...
										<p className="text-xs text-amber-700 mt-1">
											Your scene inputs changed and haven’t been saved to this
											identity yet.
										</p>
									</div>
								)}
								...
								{selectedIdentityId && !hasReferenceImages && (
									<div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
										...
										<p className="text-xs text-amber-700 mt-1">
											Add them in Account settings, then come back here to
											generate.
										</p>
									</div>
								)}
									...
									<div className="flex items-center justify-center bg-[var(--nv-lilac-white)] rounded-lg p-6 min-h-[420px] border border-[var(--nv-royal-purple)]/20">
										<div className="max-w-md text-center">
											<p className="text-[var(--nv-indigo)] font-semibold">
												{selectedIdentityId
													? "Ready when you are."
													: "Start by selecting an identity."}
											</p>
