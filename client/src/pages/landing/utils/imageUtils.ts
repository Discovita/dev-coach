...
export function getResponsiveImageSrc(
  ...
  return {
    ...
    sizes: "(max-width: 640px) 280px, (max-width: 768px) 400px, (max-width: 1024px) 600px, (max-width: 1280px) 800px, 1080px",
  };
}
...
export function extractImageBaseName(imageUrl: string): string {
  ...
  return fileName.replace(".jpg", "").replace("_original", "").replace("_thumbnail", "").replace("_medium", "").replace("_large", "");
}
