...
const TestScenarioEditor = ({
	...
	const [identityImageFiles, setIdentityImageFiles] = useState<
		Map<number, File>
	>(new Map());
	...
	useEffect(() => {
		if (scenario) {
			...
			if (
				...
				setChatMessages(
					scenario.template.chat_messages as TestScenarioChatMessage[],
				);
			} else {
				setChatMessages([]);
			}
		...
		try {
			const template: TestScenarioTemplate = {
				...
				coach_state: {
					...
					identity_focus:
						coachState.identity_focus || IdentityCategory.PASSIONS_AND_TALENTS,
			...
			onSave({
				...
				template,
				imageFiles:
					identityImageFiles.size > 0 ? identityImageFiles : undefined,
			});
		} catch (err) {
		...
		>
			...
			<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
				...
				<TabsContent value="actions">
					<TestScenarioActionsForm value={actions} onChange={setActions} />
				</TabsContent>
				<TabsContent value="breaks">
					<TestScenarioBreaksForm value={breaks} onChange={setBreaks} />
				</TabsContent>
			</Tabs>
