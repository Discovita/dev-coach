...
export default function TestScenarioActionsForm({ value, onChange }: Props) {
	...
	const handleSave = () => {
		...
		if (editingIndex !== null) {
			const updated = value.map((action, i) =>
				i === editingIndex ? actionToSave : action,
			);
			onChange(updated);
			setEditingIndex(null);
			setDraft(emptyAction());
							...
							<div className="flex-1">
								<div className="flex items-center gap-2">
									{action.result_summary ? (
										<span className="font-medium">{action.result_summary}</span>
									) : (
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div>
						...
						<Input
							...
							onChange={(e) =>
								setDraft((d) => ({ ...d, result_summary: e.target.value }))
							}
					...
					<div className="md:col-span-2">
						...
						<Textarea
							...
							onChange={(e) =>
								setDraft((d) => ({
									...d,
									coach_message_content: e.target.value,
								}))
							}
