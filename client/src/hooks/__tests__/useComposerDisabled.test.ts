...
function mockCoachState(state: Partial<CoachState> | undefined = {}) {
  vi.mocked(useCoachState).mockReturnValue({
    coachState: state === undefined ? undefined : { ...baseCoachState, ...state },
    isLoading: false,
    isError: false,
...
function coachMsg(content: string, componentConfig?: Message["component_config"]): Message {
  return {
    role: "coach",
    content,
...
describe("useComposerDisabled", () => {
  ...
  describe("truth table: on_break × latest_is_unacked_video × processing", () => {
    const cases: Array<{
      ...
      { onBreak: false, unackedVideo: false, processing: false, expected: false },
