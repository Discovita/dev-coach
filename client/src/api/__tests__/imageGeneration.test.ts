...
describe("startImageChat", () => {
  ...
  it("uses public endpoint when no user_id", async () => {
    const mockData = { image_base64: "abc", identity_id: "id-1", identity_name: "Explorer" };
    vi.mocked(authFetch).mockResolvedValue({
      ok: true,
      json: () => Promise.resolve(mockData),
    } as Response);
...
describe("continueImageChat", () => {
  ...
  it("uses public endpoint when no user_id", async () => {
    vi.mocked(authFetch).mockResolvedValue({
      ok: true,
      json: () =>
        Promise.resolve({ image_base64: "abc", identity_id: "id-1", identity_name: "X" }),
    } as Response);
  ...
  it("throws ImageGenerationError on failure", async () => {
    ...
    await expect(
      continueImageChat({ edit_prompt: "test" })
