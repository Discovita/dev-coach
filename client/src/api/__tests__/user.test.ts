...
describe("user API", () => {
	...
	describe("fetchUserComplete", () => {
		...
		it("throws on non-ok response", async () => {
			vi.mocked(authFetch).mockResolvedValue(mockErrorResponse());
			await expect(fetchUserComplete()).rejects.toThrow(
				"Failed to fetch complete user info",
			);
		});
	});
	...
	describe("fetchCoachState", () => {
		...
		it("throws on non-ok response", async () => {
			vi.mocked(authFetch).mockResolvedValue(mockErrorResponse());
			await expect(fetchCoachState()).rejects.toThrow(
				"Failed to fetch coach state",
			);
		});
	});
	...
	describe("fetchChatMessages", () => {
		...
		it("throws on non-ok response", async () => {
			vi.mocked(authFetch).mockResolvedValue(mockErrorResponse());
			await expect(fetchChatMessages()).rejects.toThrow(
	...
	describe("resetChatMessages", () => {
		...
		it("throws on non-ok response", async () => {
			vi.mocked(authFetch).mockResolvedValue(mockErrorResponse());
			await expect(resetChatMessages()).rejects.toThrow(
				"Failed to reset chat messages",
			);
		});
	});
});
