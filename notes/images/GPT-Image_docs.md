...
The image generation tool in responses supports `gpt-image-1` and `gpt-image-1-mini`. We're working on support for `gpt-image-1.5`. For a list of mainline models that support calling this tool, refer to the [supported models](/docs/guides/image-generation#supported-models) below.
