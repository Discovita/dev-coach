...
## Onward Phase — March 3 Meeting w/ Leigh Ann

### Phase 1: Data Foundation
[ ] New database table for identity commitments
    - Store the user's chosen identity focus for the week
    - Store specific actions they commit to (e.g. "dance twice this week")
    - Include time/duration metadata for each commitment
    - Coach loads these into the prompt on subsequent conversations
[ ] Track time since last conversation
    - Coach needs to know how long it's been since the user last spoke
    - Enables follow-ups like "it's been a few days, how did the dancing go?"
    - Enables greetings like "missed you yesterday"
[ ] Background task: categorize user notes by identity
    - Use existing Celery infrastructure
    - Tag existing user notes to specific identities where possible
    - Coach pulls identity-relevant facts when discussing that identity
    - Suggestions need to feel personal, not generic

### Phase 2: Onward Coaching Conversation
[ ] Build the Onward conversation phase
    - Reads/reinforces the user's identity statements
    - Asks user to pick ONE identity to focus on for the week
    - Helps them choose concrete actions for that identity
    - Follows up on previous commitments using stored data
    - Always positive reinforcement — assumes success, never punitive
    - On "failure," responds with encouragement and unwavering belief
    - Allow user to switch identities mid-week if they feel ready
    - Stick with current chat framework (no agent rewrite yet)

### Phase 3: Timer & Progress UX
[ ] 10-minute conversation timer
    - Fixed 10 minutes to start
    - Gentle notification when time is up: "feel free to finish up or keep going"
    - Not a hard stop — just a nudge
    - User can continue as long as they want after the timer
[ ] Progress bar across coaching phases
    - Visual indicator showing where the user is across all phases
    - Gives a sense of destination and completion
    - Important for short sessions — users want to see they're getting somewhere

### Phase 4: Gamification & Art
[ ] Lock/unlock artwork by phase
    - Keep identity art locked until user reaches the appropriate phase
    - Unlocking serves as a reward / dopamine hit
[ ] Teaser art generation at phase milestones
    - After "Get to Know You" or "Who I Am" phase, auto-generate a personalized image
    - Symbolizes who they are and who they want to be
    - Motivational preview before full identity art
