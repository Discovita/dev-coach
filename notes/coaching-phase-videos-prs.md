...
| 17  | FE: video modal threshold gate + action dispatch                            | `casey/cpv-17-fe-session-video-modal-action` | `[👀]` | casey 2026-05-25 | [#106](https://github.com/Discovita/dev-coach/pull/106) | 6, 7, 16 |
...
### 2026-05-25 — casey — PR 17

- **Welcome card was missing `buttons` — load-bearing fix bundled into PR 17.** PR 12 / PR 20 built the welcome card's `ComponentConfig` inline (no buttons). Once the modal grew a Continue button, that Continue had nothing to dispatch on the welcome flow. Fix: `ensure_initial_message_exists` now uses `intro_component_for("welcome_session")` from `session_video_helpers` — same builder as transition_phase / end_break, so the welcome card ships with the full `Continue → ACK(welcome_session_intro)` action chain. One source of truth for SESSION_VIDEO construction.
- **Server is the source of truth for action chains; FE never decides.** `SessionVideoCard` reads `config.buttons[0].actions ?? []` and forwards verbatim. Intros (single `[ACK]`) and outros (`[ACK, START_BREAK]`) flow through the same code path. If the backend's helpers ever extend the chain, the FE picks it up automatically.
- **`{message: null, actions: [...]}` dispatch needed three FE call-sites updated** beyond just the type change: (a) `CoachRequest.message` type to `string | null`, (b) `ChatInterface.handleSendMessage` to bypass the empty-string guard when `message === null`, (c) `use-chat-messages.ts onSuccess` to skip the optimistic user bubble when `message === null` (mirrors the backend not saving a user ChatMessage). Without (b), the click would be a no-op; without (c), the chat would show a phantom `user: null` row until the next refetch.
- **`SessionVideoModal` only renders Continue when `!acknowledged && actions.length > 0`.** The `actions.length > 0` guard is defensive against any stale/historical SESSION_VIDEO row whose `component_config.buttons` somehow ended up empty — render the modal as a "Watch Again"-style replay-only view rather than ship a broken Continue button. In practice every card built today has actions, but the guard makes the code resilient to schema-change footguns.
- **`<SessionVideoModal key={open ? "open" : "closed"}>` resets `useVideoThreshold` + the `<video>` element on each reopen.** Without the key, the hook's `thresholdReached` would stay true between modal opens (sticky once-reached), so a user who scrubbed close to the end, closed the modal, and reopened it would see Continue immediately enabled. Re-mounting via the key is cheaper than wiring an external reset. Worth a flag for PR 18 / future modal work: prefer the React key pattern for "reset on reopen" semantics.
- **`useVideoThreshold` is decoupled from the `<video>` element on purpose.** The hook takes `(currentTime, duration)` as plain numbers from the `onTimeUpdate` event, not a ref. Easier to test (just call `onTimeUpdate(99, 120)` directly) and lets the hook be reused if we ever add a custom player later.
- **`fireTimeUpdate` test helper redefines `currentTime` and `duration` on the video element via `Object.defineProperty`** — jsdom's `HTMLVideoElement` doesn't compute these from a video source, so the native `timeupdate` event would fire with `0/NaN`. Vitest pattern for any future video-element interaction tests.
- **Frontend at 228 / Backend at 811 after PR 17** (FE was 211, +17; BE was 810, +1).
...
- **Bucket name lookup needed a top-level setting.** Django doesn't expose a top-level `AWS_STORAGE_BUCKET_NAME` setting — the bucket is buried in `STORAGES["default"]["OPTIONS"]["bucket_name"]`, which (a) isn't easy to override in tests and (b) doesn't exist in `settings.test` because that file extends `common.py` and `STORAGES` is only defined in `local.py` / `staging.py` / `production.py` / `previews.py`. Fix: added a top-level `AWS_STORAGE_BUCKET_NAME` setting in `common.py` (defaulting to the staging bucket via the existing `STAGING_AWS_STORAGE_BUCKET_NAME` env var) and overridden it in `production.py`. Tests pin via `@override_settings`. This is the single setting `get_video_url` reads.
- **Registry shape changed from `{name, url}` to `{name, s3_key}`.** Storing the bucket-relative path (instead of a full URL) lets the same registry serve staging and production by resolving the URL per-request against the env's bucket. Previously the `url` field was just `""` (placeholder for PR 20 fill) — now there's no placeholder, the resolution is computed.
- **`url` literal is intentionally NOT stored anywhere in the registry.** A reviewer might expect `s3_key` to map 1:1 to a literal URL field in the registry. It doesn't — the URL is purely a runtime composition of `settings.AWS_STORAGE_BUCKET_NAME` + the s3_key. This avoids the staging-vs-production fork.
- **`AWS_STORAGE_BUCKET_NAME` collides with django-storages' historical setting name.** Django-storages also recognizes a top-level `AWS_STORAGE_BUCKET_NAME` (legacy API; this project uses the newer `STORAGES["default"]["OPTIONS"]["bucket_name"]` form instead, but django-storages still reads the top-level constant if present). Setting it has no observable effect here because the `STORAGES` dict in each env file takes precedence and re-specifies `bucket_name` inside `OPTIONS`. Worth flagging in case a future django-storages upgrade changes precedence.
- **Persisted `component_config.video_url` is bucket-snapshot-locked.** If the bucket name ever changes, historical ChatMessage rows still carry the old URL until they're rebuilt. Not a problem today (buckets are stable per env), but a future bucket migration would need a one-off management command to backfill. Documented in the field's pydantic description so the next reader doesn't get burned.
- **Three call sites had to be updated identically.** `intro_component_for`, `outro_component_for` (both in `session_video_helpers.py`), and `ensure_initial_message_exists` (the welcome card seed) all construct SESSION_VIDEO `ComponentConfig`s. Each had to grow the `video_name=get_video(key)["name"]` + `video_url=get_video_url(key)` pair. No central builder yet — would be a reasonable next refactor if a fourth construction site ever appears.
- **Upload script avoids Django bootstrap.** `scripts/upload_session_videos.py` adds `server/` to `sys.path` and imports `SESSION_VIDEOS` directly. It doesn't `django.setup()`. That's intentional — the script is ops tooling, not request-path code, and the registry module is pure Python. Importing the registry pulls in `apps.coach_states.constants.session_videos` which imports `django.conf.settings` — only used by `get_video_url`, NOT by the registry dict the script reads. The script never calls `get_video_url`, so no Django bootstrap is needed.
- **Script uploads to BOTH buckets in one invocation by default.** Reasoning: a single operator session covers every env. Per-bucket targeting available via `--bucket {staging,production}` for surgical re-uploads. ACL not set — both buckets disable ACLs and rely on bucket-policy public access (matches the existing user-images storage convention).
- **`ContentType: video/quicktime` + `CacheControl: public, max-age=31536000, immutable`** set on every upload. Videos are immutable content (rename them in the registry to bust caches if needed) and benefit from aggressive caching — 1 year is the standard "immutable static asset" TTL.
- **Backend suite at 810 after PR 20** (was 801 after PR 14, +9 here). FE suite 200 passing, `tsc --noEmit` clean.
- **PR 19 also marked `[✓]` in this branch's notes update.** Combined the two row updates into PR 20's notes commit because PR 19 had no actual commit of its own.
...
### 2026-05-25 — casey — PR 10

- **`Action.coach_message` is now nullable.** Not explicitly in PR 10's plan, but mechanically required by the null-message contract: when `message=None`, the orchestrator passes `user_chat_message=None` to `apply_user_component_actions`; the three user-button-only handlers (`ACKNOWLEDGE_SESSION_VIDEO`, `START_BREAK`, `END_BREAK`) each write `Action.objects.create(coach_message=<that anchor>)`. Without nullability that crashes at runtime as soon as a real ACK turn lands (PR 12 onward). The Break model's `coach_message` already uses the same pattern (PR 4 Discoveries). Added `null=True, blank=True` + migration `0009_alter_action_action_type_alter_action_coach_message_and_more.py`.
- **The new actions migration also rolls up the pre-existing `action_type` + `timestamp` drift** that has been flagged on `main` since PR 3 (`makemigrations --check --dry-run` was noisy after every backend PR). Django generated all three field alterations into one file because nothing had been written for them yet. Net effect: post-PR-10, the only remaining drift is `prompts/0020_alter_prompt_allowed_actions.py`. Future backend PRs can stop ignoring the actions one.
- **`apply_user_component_actions` signature updated to `Optional[ChatMessage]`.** Pure type-annotation change; the body already passes the value straight through to `apply_component_actions`, which forwards to handlers that now accept `None`.
- **`CoachRequestSerializer.message` is `required=False, allow_null=True, allow_blank=True`.** Cross-field `validate()` is unchanged — `not message and not actions` still rejects truly empty bodies. The existing `test_actions_only_without_message_is_invalid` was renamed to `..._is_valid` and its assertion inverted; two new tests cover the explicit `{message: null, actions: [...]}` and `{message: null}`-alone cases.
- **Skip-LLM path persists the user-action's component on a fresh coach `ChatMessage` with `content=""`.** Spec lifecycle step 5 ("the break card is saved as the next coach message") was the guide. The orchestrator writes via `add_chat_message(user, "", COACH)` then sets `component_config = user_component_config.model_dump()` + `save(update_fields=["component_config"])`. `build_coach_response_data` is then called with `coach_message=""` and `final_prompt=""` — the coach response carries the component but no LLM text. PR 11's history serialization will render this row as bracketed narration for the LLM's next turn.
- **Test helper `_make_user()` had to pin `user.breaks.filter().exists()` to `False`** explicitly. `MagicMock` chains auto-return more MagicMocks, which are truthy → would flag the user as `on_break=True` and confuse builder assertions. Forcing the chain to `False` in the helper makes the tests reflect the default "no open break" state. Any future test that wants the on-break case can override `user.breaks.filter.return_value.exists.return_value = True`.
- **Existing tests' `_make_process_message_mocks` had to set `apply_user_component_actions.return_value = None` explicitly.** Previously the mock returned a bare MagicMock (truthy) — that was harmless before because the return value was unused. Now the orchestrator branches on it; a truthy MagicMock would have falsely triggered the skip-LLM path in every happy-path test.
- **Backend suite at 772 after PR 10** (up from 763 after PR 9; +9: 4 null-message-contract, 3 skip-LLM-rule, 1 regression, 2 new serializer cases — minus 1 renamed/inverted existing test that doesn't increase the count).

### 2026-05-25 — casey — PR 11

- **The plan's `get_recent_chat_messages_for_prompt` rewrite alone wouldn't have reached the LLM.** The orchestrator passes its result to `generate_coach_ai_response` → `openai_service.generate(...)`, which **omits** `chat_history` at the API boundary — recent messages are embedded in the prompt by the `PromptManager` via `services/prompt_manager/utils/context/func/get_recent_messages_context.py`. PR 11 therefore updates BOTH places: the new `serialize_chat_message_for_prompt` helper is the single source of truth, and `get_recent_messages_context` now calls it. Both functions also bump their default count from 5 → 10.
- **Function contract changed: `get_recent_chat_messages_for_prompt` now returns `list[str]`** (was `list[ChatMessage]`). The chain `generate_coach_ai_response` → `AIService.generate` → `OpenAIService.generate` had its `chat_history` annotation propagated to `list[str]`. `chat_history=[]` is still the value tests pass, so no test breakage. The `apps.chat_messages.models.ChatMessage` import was dropped from `services/ai/openai_service.py` (now unused) and from `services/ai/base.py` (now type-string only).
- **`video_key` is read from `component_config["video_key"]`.** PR 11's serializer reads it directly from the JSON dict. PR 13 (which emits the SESSION_VIDEO component from the `transition_phase` handler) is responsible for populating that key when constructing the `ComponentConfig`. The `ComponentConfig` Pydantic model does NOT yet declare a `video_key` field — left for PR 13 to add cleanly alongside the actual construction site. PR 11's tests build component_config dicts by hand with `video_key`, so they don't depend on the model field.
- **`Break` lookup keyed by `coach_message_id` (the column, not the FK)** — `Break.objects.filter(coach_message_id__in=message_ids)`. Filters by the FK column directly so a single query covers the whole batch; no per-row N+1. The result maps `coach_message_id → Break`, which the serializer indexes by `msg.id`.
- **Circular-import landmine in `get_recent_messages_context`.** Importing `serialize_chat_message_for_prompt` at module top loads `apps.coach.utils.__init__`, which imports `build_coach_prompt`, which transitively imports `services.prompt_manager.manager`, which imports `get_recent_messages_context` — full cycle. Fixed with a deferred import inside the function body. The comment block at the top of the file explains the constraint so the next agent doesn't move it.
- **Backend suite at 779 after PR 11** (up from 772 after PR 10; net +7 — 13 in the targeted file (4 carried over + 9 new), but the old file had 7 tests and three of those had to be reworked to reflect the new contract). All 9 plan-required tests pass: acked/unacked video, closed/open break, text passthrough, text+component-on-same-row, default count 10, custom count override, registry name lookup.

### 2026-05-25 — casey — PR 12

- **Existing tests live at `apps/users/tests/test_ensure_initial_message_exists.py`,** not `apps/chat_messages/tests/`. The plan's "files likely touched" guessed `apps/chat_messages/tests/test_ensure_initial_message_exists.py` would exist or be created — in reality the legacy file was placed under `apps/users/tests/` (likely because `ensure_initial_message_exists` is most-used by user-facing endpoints). Extended in place rather than creating a parallel file under `apps/chat_messages/tests/` — having two files for the same util would be confusion-inviting. Future PR-24 cleanup should expect to delete or trim the legacy `INITIAL_MESSAGE`-patching tests in the same file.
- **Plan's "Open decision" resolved as Option A: added `video_key: Optional[str] = None` to the `ComponentConfig` pydantic model.** PR 11 reads `component_config["video_key"]` from the JSON dict at serialization time, but the model didn't have the field. Adding it in PR 12 keeps the model in sync with the data on disk and lets the welcome-card emission use `ComponentConfig(...).model_dump()` instead of a hand-built dict. PR 13 should construct SESSION_VIDEO components via `ComponentConfig(component_type=..., video_key=...)` directly — no more raw-dict construction for the video flow.
- **`add_chat_message` doesn't accept `component_config`,** so the welcome-card emission uses the same two-step write that PR 10's skip-LLM path uses: `add_chat_message(user, "", COACH)` → set `component_config = config.model_dump()` → `save(update_fields=["component_config"])`. Doing it via `ChatMessage.objects.create(...)` directly would have worked too, but mirroring the existing pattern keeps the seed path consistent with the runtime path.
- **`pydantic.model_dump()` includes `texts=None`, `buttons=None`, `identities=None`** in the serialized component_config (no `exclude_none=True`). Same shape PR 10's skip-LLM path produces, and PR 11's serializer just inspects the keys it cares about — no consumer breakage.
- **`WELCOME_VIDEO_KEY = "welcome_session_intro"`** is a module-level constant in `ensure_initial_message_exists.py` rather than a literal at the call site. Tests import it to assert key equality without re-typing the string. PR 13 / PR 14 should consider whether the video key should be derived from `SESSIONS["welcome_session"]["intro"]` instead — for PR 12 the literal is fine because PR 13 is going to need a per-transition lookup anyway and centralizing that is its scope, not this one's.
- **No `apps/chat_messages/tests/` collection error.** The existing `apps/chat_messages/tests/` directory has its own test files (`test_add_chat_message.py`, `test_chat_message_model.py`, `test_chat_message_serializer.py`, `test_signal.py`) — none of them currently target `ensure_initial_message_exists`. Sticking the new tests in the existing file under `apps/users/tests/` keeps a single canonical location for util-level tests.
- **Backend suite at 784 after PR 12** (up from 779 after PR 11; net +5 — 9 in the targeted file (4 carried over + 5 new)). The new tests are the 4 plan-required ones (with idempotency split into flag-on and flag-off variants so each branch has its own regression case).
- **Pre-existing pending migration on `prompts/0020_alter_prompt_allowed_actions.py`** still flagged by `makemigrations --check --dry-run` — unrelated to PR 12. PR 10 rolled up the `actions/0009` drift; the prompts one remains.

### 2026-05-25 — casey — PR 15

- **Composer lives at `client/src/pages/chat/components/ChatControls.tsx`.** The PR 15 plan flagged this as a discovery — `ChatInterface.tsx` (the chat page shell) wraps it but the `Textarea` + Send `Button` are inside `ChatControls`. PR 18 (unacked-SESSION_VIDEO disable clause) edits the same `composerDisabled` derivation in this file. The plan also suggests extracting a `use-composer-disabled` hook in PR 18 once there are two clauses — agreed.
- **Disable rule lives inline as `composerDisabled = isProcessingMessage || coachState?.on_break === true`** — not a new hook. One clause didn't justify the indirection. The `Sending...` label still keys on `isProcessingMessage` only (on_break has no spinner semantics), so kept those decoupled.
- **Optimistic `on_break` cache mirror in `use-chat-messages.ts` `onSuccess`.** After the mutation succeeds, `setQueryData<CoachState>([...queryKeyPrefix, "coachState"], old => ({ ...old, on_break: response.on_break }))` patches the cached coach state immediately, then the existing `invalidateQueries` triggers a refetch as confirmation. Without the mirror there's a brief flash between the response landing and the refetch finishing where the composer is in the wrong state. PR 17/18 (component-action dispatch for video Continue / break I'm Ready) should apply the same mirror in their mutation `onSuccess` so START_BREAK / END_BREAK button clicks flip the composer instantly too.
- **`on_break` is `boolean | undefined` (optional) on both types**, not required. Two reasons: (a) older backend responses pre-PR 9 may omit it, (b) the field is derived backend-side (`Break.objects.filter(...).exists()`) so a serializer chain change could drop it. The composer's `=== true` predicate is the safe check — `undefined` and `false` both render the composer enabled.
- **Cache key for `setQueryData`: `[...queryKeyPrefix, "coachState"]`** — `queryKeyPrefix` switches between `["user"]` and `["testScenarioUser", targetUserId]` via the `UserTargetContext` (the admin-impersonation pattern, see memory `79920969`). Both code paths get the optimistic update automatically because they share the key construction.
- **Bulletin components stubbed in the ChatControls test.** `WarmupBulletin` / `BrainstormingBulletin` / `RefinementBulletin` / `CommitmentBulletin` are pure renders of coachState/identities; mocked to `() => null` so the test focuses on composer behavior without dragging in their internals.
- **`useChatMessages.test.tsx` is the first hook-level test for that hook.** Mocks all five upstream modules (`@/api/user`, `@/api/testScenarioUser`, `@/api/coach`, `@/context/UserTargetContext`) and uses `createQueryWrapper` + `renderHook` + `act` + `waitFor` to assert the cache mirror lands. Mirror this setup for future hook tests on this file (PR 17/18 component-action dispatch).
- **Frontend suite at 200 after PR 15** (up from 190 — net +10: 2 in useCoachState, 5 in new ChatControls.test.tsx, 3 in new useChatMessages.test.tsx).
- **`npx tsc --noEmit` clean.** The optional `on_break?: boolean` on `CoachResponse` flows through `apiClient.sendMessage` → mutation onSuccess without any other call-site changes.

### 2026-05-25 — casey — PR 14

- **Handler does NOT write `coach_message.component_config` — opposite of PR 13.** END_BREAK is a user-button-only action dispatched via `apply_component_actions` (with `coach_message` actually being the USER's "I'm ready" message). The orchestrator's skip-LLM-on-component branch in `apps/coach/functions/public/process_message.py` is the side that creates the new coach `ChatMessage(content="")` and writes `component_config = user_component_config.model_dump()` on it (locked in by PR 10 — see PR 10 Discoveries). Writing to `coach_message.component_config` in this handler would mutate the user message, which is exactly the bug we'd be wallpapering over. Pattern reference for component-returning user-button handlers: `start_break.py` returns ComponentConfig without touching `coach_message.component_config`. So three categories now: PR 13's `transition_phase` (LLM-path; persists), this PR's `end_break` (user-button-only; orchestrator persists), and SHOW_* (response-only; nobody persists).
- **`end_break.py` stayed at `actions/` top level, not moved to `persistent_components/`.** The PR 14 plan flagged the move as optional. Dispatcher uses `ACTION_REGISTRY`, not the file path. Test imports (`from services.action_handler.actions.end_break import end_break`) and the existing PR 8 test layout would have churned for zero behavior change. Future component-returning user-button handlers can follow either convention — pick by reader-grep convenience, not by dispatcher requirement.
- **Removed PR 8's `test_end_break_returns_no_component_config_in_this_pr`.** Its purpose was to lock the PR 8 / PR 14 split (handler always returns None until PR 14 inverts it). PR 14 is the inversion, so the test would have started failing if kept. Other PR 8 tests carry forward unchanged.
- **Integration test mirrors PR 13's structure but routes through `apply_user_component_actions` instead of `apply_coach_actions`.** END_BREAK is user-button-only — the LLM never emits it, so the LLM-path `apply_coach_actions` would be the wrong entrypoint. Dispatches `[{"action": ActionType.END_BREAK.value, "params": {}}]` through `apply_user_component_actions` and asserts the returned `Optional[ComponentConfig]` is a `SESSION_VIDEO` carrying the right `video_key`. This is the contract PR 10's skip-LLM rule consumes.
- **Parametrized post-break boundary table is 5 entries**, one per session that has an outro (and therefore triggers a break that lands the user at the first phase of the next intro-bearing session): get_to_know → brainstorming, brainstorming → refinement, refinement → commitment, commitment → i_am, i_am → visualization. The `welcome_session` is intro-only (no outro → no break → no end_break intro path) and `visualization_session` is the terminal session, so the boundary table caps at 5 — same shape as PR 13's `_OUTRO_BOUNDARIES` table.
- **Backend suite at 801 after PR 14** (up from 794 after PR 13; net +7 — 4 PR 8 carried forward + 8 new PR 14 minus 1 removed PR 8 lock test = 11 in file, plus the obsolete one removed).
- **Pre-existing pending migration on `prompts/0020_alter_prompt_allowed_actions.py`** still flagged by `makemigrations --check --dry-run` — unrelated to this PR.

### 2026-05-25 — casey — PR 13

- **Handler also writes `coach_message.component_config` to DB,** not just returns the `ComponentConfig`. PR 11's history serializer reads `msg.component_config` from DB to render bracketed narration on the next LLM turn, so the persistence is load-bearing — not a cosmetic redundancy with `build_coach_response_data`. Mirrors the `persistent_components/persist_*.py` handlers. SHOW_* handlers don't persist (intentional — they're response-only and don't replay), so the SESSION_VIDEO path is a third category: returned + persisted. Future video/break component handlers should follow the same pattern; future SHOW-style handlers should NOT.
- **Shared helpers live at `apps/coach_states/utils/session_video_helpers.py`** (new module + new `utils/` package in the `coach_states` app). Predicate API: `should_emit_outro(old_phase) → bool` and `should_emit_intro(new_phase, shown_videos) → bool`. Builder API: `outro_component_for(leaving_session) → ComponentConfig` and `intro_component_for(entering_session) → ComponentConfig`. PR 14 reuses both — calling `should_emit_intro(coach_state.current_phase, coach_state.shown_videos)` + `intro_component_for(session_of(coach_state.current_phase))` covers the post-break flow without duplicating logic.
- **`_try_session_of` defensive against `SYSTEM_CONTEXT`.** The shared `session_of(phase)` raises `ValueError` for any phase not in the SESSIONS map (SYSTEM_CONTEXT is the only such phase today, but future meta-phases would hit this). The handler shouldn't crash on those — it should just emit no video. Wrapping in try/except → None in the helpers keeps the call sites clean.
- **Outro is NOT idempotency-checked** (no `shown_videos` membership test). The spec is intentional — a session's outro fires the one time the user leaves that session; there's no flow in the runtime today that re-enters and re-leaves the same session. If a future feature changes that, the predicate should grow an additional check. Intros ARE idempotency-checked (the welcome video remains in `shown_videos` after first ACK, so a hypothetical re-entry doesn't re-show the welcome card).
- **Integration test routes through `apply_coach_actions`,** not directly through `transition_phase`. Builds a real `CoachChatResponse(message=..., transition_phase=TransitionPhaseAction(params=TransitionPhaseParams(...)))` and asserts that the dispatcher hands back a `(state, component_config)` tuple where the component is the SESSION_VIDEO card. Verifies the wiring from LLM-emitted action → dispatcher → orchestrator stays intact for the new return-type contract. Mirror this pattern in PR 14's integration test (route through `apply_component_actions` since `END_BREAK` is user-button-only).
- **`subTest`-based parametrization for outro emission across all 5 outro-bearing sessions.** Django TestCase doesn't have pytest's `@parametrize`, so the loop uses `with self.subTest(session=...)` for distinct failure messages per session. `ChatMessage.objects.all().delete()` + `Action.objects.all().delete()` between iterations keeps each sub-test's assertions independent. Reuse this idiom in PR 14 for the per-session intro emission tests.
- **Backend suite at 794 after PR 13** (up from 784 after PR 12; net +10). 16 tests in the targeted file (6 carried over + 10 new). All 9 plan-required tests pass + the integration test through `apply_coach_actions`.
- **Pre-existing pending migration on `prompts/0020_alter_prompt_allowed_actions.py`** still flagged by `makemigrations --check --dry-run` — unrelated to PR 13.
