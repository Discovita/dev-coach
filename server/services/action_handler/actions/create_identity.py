...
def create_identity(
    coach_state: CoachState, params: CreateIdentityParams, coach_message: ChatMessage
):
    """
    ...
    existing_identity = Identity.objects.filter(
        user=coach_state.user, name__iexact=params.name
    ).first()

    if existing_identity:
        log.warning(
            f"Identity with name '{params.name}' already exists for user {coach_state.user.id}. Skipping creation."
        )
        return None
