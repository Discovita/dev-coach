...
def persist_combine_identities(
    coach_state: CoachState,
    params: PersistCombineIdentitiesParams,
    user_message: ChatMessage,
):
    """
    Persist the combine identities component configuration to the coach message for historical display.
    ...
    Action.objects.create(
        user=coach_state.user,
        action_type=ActionType.PERSIST_COMBINE_IDENTITIES.value,
        parameters=params.model_dump(),
        result_summary=(
            f"Persisted combine identities component for identities {params.identity_id_a} and {params.identity_id_b}"
        ),
