...
def _resize_if_needed(
    image: PILImage.Image, max_dim: int = MAX_IMAGE_DIMENSION
) -> PILImage.Image:
    """
...
def load_pil_images_from_references(
    ...
    for idx, ref_image in enumerate(reference_images):
        log.debug(
            f"Processing reference image {idx + 1}/{len(reference_images)}: {ref_image.id}"
        )
        ...
        try:
            ...
            log.debug("Converting to PIL Image...")
            ...
            log.info(
                f"Successfully loaded reference image {idx + 1}/{len(reference_images)} ({pil_image.size[0]}x{pil_image.size[1]})"
            )

        except Exception as e:
            log.error(
                f"Failed to load ReferenceImage {ref_image.id}: {e}", exc_info=True
            )
            continue

    log.info(
        f"Finished loading reference images: {len(pil_images)} successful out of {len(reference_images)}"
    )
    return pil_images
