...
def generate_character_views(
    client: genai.Client, source_paths: list[str], output_name: str | None = None
) -> tuple[str, list[str]]:
    """
    ...
    for i, source_path in enumerate(
        source_paths[:5]
    ):  # Max 5 for character consistency
...
def main():
    ...
    if len(source_image_paths) < 1:
        print(
            "Usage: python generate_character_views.py <source_image> [source_image2 ...] [--output <directory_name>]"
        )
        print()
        print(
            "This script takes one or more user photos and generates 5 clean headshot views:"
        )
        ...
        print("Options:")
        print(
            "  --output <name>  Custom name for output directory (default: numbered 01, 02, etc.)"
        )
        print()
        print("Examples:")
        print("  # Single image")
    ...
    output_dir, generated_paths = generate_character_views(
        client, source_image_paths, output_name
    )
