    ...
    def to_representation(self, instance):
        if instance:
            try:
                ...
                log.debug("Successfully generated all image URLs")
                return sizes
            except Exception as e:
                log.error(f"Error generating image URLs: {e}", exc_info=True)
                return None
        else:
            log.debug("No instance provided, returning None")
            return None
