...
class TemplateUserSerializer(ForbidExtraFieldsMixin, serializers.Serializer):
    ...
    email = serializers.EmailField(
        required=False,
        help_text="User's email address. If not provided or not unique, a unique test email will be generated automatically.",
    )
    password = serializers.CharField(
        required=False,
        help_text="Password for the test user. Not required; always set to 'Coach123!' by the backend.",
    )
    ...
    is_superuser = serializers.BooleanField(
        required=False, help_text="Is the user a superuser?"
    )
    is_staff = serializers.BooleanField(required=False, help_text="Is the user staff?")
    verification_token = serializers.CharField(
        required=False, allow_blank=True, help_text="Token for email verification."
    )
    email_verification_sent_at = serializers.DateTimeField(
        required=False,
        allow_null=True,
...
class TemplateCoachStateSerializer(ForbidExtraFieldsMixin, serializers.Serializer):
    ...
    identity_focus = serializers.CharField(
        help_text="Identity focus for the coaching session. Required."
    )
    who_you_are = serializers.ListField(
        child=serializers.CharField(),
        help_text="List of 'who you are' identities. Required.",
    )
    ...
    skipped_identity_categories = serializers.ListField(
        child=serializers.CharField(),
        required=False,
        help_text="List of skipped identity categories.",
    )
    proposed_identity = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="ID of the currently proposed identity.",
    )
    current_identity = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="Name of the current identity being refined.",
    )
...
class TemplateIdentitySerializer(ForbidExtraFieldsMixin, serializers.Serializer):
    ...
    category = serializers.CharField(
        help_text="Category this identity belongs to. Required."
    )
...
class TemplateUserNoteSerializer(ForbidExtraFieldsMixin, serializers.Serializer):
    ...
    source_message = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="ID of the chat message that prompted this note.",
    )
