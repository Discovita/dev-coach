    ...
    try:
        ...
        if s3_domain in image_url or (
            ...
            if "/media/" in image_url:
                source_key = image_url.split("/media/")[1].split("?")[
                    0
                ]  # Remove query params
        ...
        if is_s3_url and source_key:
            ...
            if not actual_source_key:
                log.error(
                    f"Image not found in S3 with any of these keys: {possible_keys}"
                )
                return None
            ...
            # Copy object within S3
            copy_source = {"Bucket": bucket_name, "Key": actual_source_key}

            log.info(f"Copying image within S3 from {actual_source_key} to {dest_key}")
            s3_client.copy_object(
                CopySource=copy_source, Bucket=bucket_name, Key=dest_key
            )
