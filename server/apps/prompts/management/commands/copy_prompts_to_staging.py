...
class Command(BaseCommand):
    help = (
        "Copy all prompts from local to staging database (OVERWRITES staging prompts)"
    )
    ...
    def handle(self, *args, **options):
        ...
        result = subprocess.run(
            count_cmd, capture_output=True, text=True, env=local_env
        )
        ...
        result = subprocess.run(
            delete_cmd, capture_output=True, text=True, env=staging_env
        )
        if result.returncode != 0:
        ...
        result = subprocess.run(
            restore_cmd, capture_output=True, text=True, env=staging_env
        )
        if result.returncode != 0:
        ...
        result = subprocess.run(
            verify_cmd, capture_output=True, text=True, env=staging_env
        )
        staging_count = result.stdout.strip()

        self.stdout.write(
            f"✅ Staging now has {staging_count} prompts (was {local_count} locally)"
        )
