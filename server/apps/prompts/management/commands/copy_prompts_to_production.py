...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        result = subprocess.run(
            count_cmd, capture_output=True, text=True, env=staging_env
        )
        ...
        result = subprocess.run(
            dump_cmd, capture_output=True, text=True, env=staging_env
        )
        if result.returncode != 0:
        ...
        result = subprocess.run(
            delete_cmd, capture_output=True, text=True, env=prod_env
        )
        if result.returncode != 0:
        ...
        result = subprocess.run(
            restore_cmd, capture_output=True, text=True, env=prod_env
        )
        if result.returncode != 0:
        ...
        result = subprocess.run(
            verify_cmd, capture_output=True, text=True, env=prod_env
        )
        prod_count = result.stdout.strip()

        self.stdout.write(
            f"✅ Production now has {prod_count} prompts (was {staging_count} in staging)"
        )
