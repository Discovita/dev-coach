...
class AdminIdentityViewSet(viewsets.GenericViewSet):
    ...
    def download_i_am_statements_pdf_for_user(self, request):
        ...
        try:
            ...
            log.info(
                f"Admin {request.user.id} downloaded I Am Statements PDF for user {target_user.id}"
            )
            return response
    ...
    def generate_image(self, request):
        ...
        try:
            ...
            return Response(
                {"error": f"User {user_id} not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def save_generated_image(self, request):
        ...
        try:
            ...
            return Response(
                {"success": True, "identity": IdentitySerializer(identity).data}
            )
