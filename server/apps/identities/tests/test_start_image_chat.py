...
class StartImageChatFunctionTests(TestCase):
    ...
    def test_start_chat_success(
        self, mock_prompt_manager, mock_load_images, mock_service_class
    ):
        """Test successfully starting a chat and returning image."""
        from apps.identities.functions.public import start_image_chat

        # Setup mocks
        mock_prompt_manager.return_value.create_image_generation_prompt.return_value = (
            "Generate an image"
        )
        ...
        mock_service.send_chat_message.return_value = (
            self._create_mock_pil_image(),
            mock_response,
        )
    ...
    @patch("services.image_generation.orchestration.load_pil_images_from_references")
    @patch("services.image_generation.orchestration.PromptManager")
    def test_start_chat_creates_record(
        self, mock_prompt_manager, mock_load_images, mock_service_class
    ):
    ...
    @patch("services.image_generation.orchestration.load_pil_images_from_references")
    @patch("services.image_generation.orchestration.PromptManager")
    def test_start_chat_replaces_existing(
        self, mock_prompt_manager, mock_load_images, mock_service_class
    ):
...
class StartImageChatAPITests(APITestCase):
    ...
    def test_start_chat_success(
        ...
        response = self.client.post(
            self.public_url, {"identity_id": str(self.identity.id)}, format="json"
        )
    ...
    @patch("services.image_generation.orchestration.load_pil_images_from_references")
    @patch("services.image_generation.orchestration.PromptManager")
    def test_admin_start_chat_success(
        self, mock_prompt_manager, mock_load_images, mock_service_class
    ):
    ...
    def test_start_chat_missing_identity_id(self):
        ...
        response = self.client.post(self.public_url, {}, format="json")
